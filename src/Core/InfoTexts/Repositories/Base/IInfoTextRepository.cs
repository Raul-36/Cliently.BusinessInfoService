...
namespace Core.InfoTexts.Repositories.Base
{
    ...
    {
        public Task<IEnumerable<InfoTextFlexDto>> GetAllByBusinessIdAsync(Guid businessId, InfoTextQueryOptions options);
        public Task<InfoText> GetByIdAsync(Guid id);
        public Task<InfoTextFlexDto> GetByIdAsync(Guid id, InfoTextQueryOptions options);
